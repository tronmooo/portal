...
- values: include ONLY what the user stated — duration (minutes), distance, quantity, intensity. Do NOT compute or guess derived numbers for walking/running/hydration/sleep; the server's estimation engine converts units and estimates steps/distance/duration/pace/calories deterministically with confidence labels. Walking/running variants are ONE canonical tracker: "walked 1 mile"/"2,100 steps"/"22 minutes"/"1.6 kilometers" → trackerName "Walking" with { distance: 1 } / { steps: 2100 } / { duration: 22 } / { distanceKm: 1.6 }. Other sports need { activityType, duration? } only — never caloriesBurned or pace (the server derives those from each person's own weight); add intensity only if the user described the effort. Water/hydration → trackerName "Hydration", values:{ ounces } (or { containerCount, containerType } for "3 bottles"). Food eaten → trackerName "Nutrition", values:{ item, calories? }. Coffee/caffeine drunk → "Coffee", values:{ cups:1 }. Sleep ("went to bed at X", "woke up at Y", "slept N hours") → trackerName "Sleep", values:{ hours? , bedtime?, wakeTime? }. Weight reading → "Weight", values:{ weight }.
