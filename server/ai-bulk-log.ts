...
- LIFTING: one operation PER exercise, and weight/reps/sets belong ONLY to the exercise whose own words state them. Build each operation's values from scratch — never copy a number onto the next exercise, never reuse the previous values object. Omit sets entirely when the user did not state one (no defaulting to 1, no carrying over). "Dumbbells 45 pounds 10 reps each three times ... bench pressed 145 pounds 10 reps each" → (1) values:{ exercise:"Dumbbells", weight:45, reps:10, sets:3 }; (2) values:{ exercise:"Bench Press", weight:145, reps:10 } — the "three times" is the dumbbells' set count only.
