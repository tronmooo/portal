...
export class SupabaseStorage implements IStorage {
  ...
  async seedIfEmpty(): Promise<void> {
    const { data } = await this.supabase.from("profiles").select("id").eq("user_id", this.userId).limit(1);
  ...
  private async seedData(): Promise<void> {
    ...
    // ---- Profiles ----
    const p1Id = randomUUID(), p2Id = randomUUID(), p3Id = randomUUID(), p4Id = randomUUID();
    const p5Id = randomUUID(), p6Id = randomUUID(), p7Id = randomUUID(), p8Id = randomUUID();

    await this.supabase.from("profiles").insert([
      { id: p1Id, user_id: uid, type: "person", name: "Mom", fields: { phone: "555-0101", relationship: "Family", birthday: "1965-03-12", email: "mom@email.com" }, tags: ["family", "emergency-contact"], notes: "Likes flowers for Mother's Day. Allergic to shellfish.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(30), updated_at: now },
      { id: p2Id, user_id: uid, type: "pet", name: "Max", fields: { breed: "Golden Retriever", age: 3, weight: "72 lbs", vetName: "Dr. Sarah Chen", vetPhone: "555-0200", microchip: "985112006547892" }, tags: ["pet", "family"], notes: "Next vaccination due April 2026. Loves peanut butter treats.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(60), updated_at: now },
      { id: p3Id, user_id: uid, type: "vehicle", name: "Tesla Model 3", fields: { year: 2024, mileage: 12500, color: "Midnight Silver", vin: "5YJ3E1EA3RF123456", insurance: "State Farm", nextService: "2026-06-01" }, tags: ["vehicle", "tesla"], notes: "Tire rotation every 7,500 miles. Supercharger membership active.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(90), updated_at: now },
      { id: p4Id, user_id: uid, type: "subscription", name: "Netflix", fields: { plan: "Premium", cost: "$22.99/mo", renewalDate: "2026-04-01", email: "user@email.com" }, tags: ["subscription", "entertainment"], notes: "", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(45), updated_at: now },
      { id: p5Id, user_id: uid, type: "medical", name: "Dr. James Park", fields: { specialty: "Primary Care", phone: "555-0300", clinic: "Coastal Health Center", lastVisit: "2026-01-15", nextVisit: "2026-07-15" }, tags: ["medical", "doctor"], notes: "Annual physical scheduled for July.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(120), updated_at: now },
      { id: p6Id, user_id: uid, type: "self", name: "Me", fields: { height: "5'10\"", bloodType: "O+", allergies: "None", emergencyContact: "Mom (555-0101)", ssn_last4: "4821" }, tags: ["self"], notes: "Primary profile. All personal health data linked here.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(180), updated_at: now },
      { id: p7Id, user_id: uid, type: "loan", name: "Auto Loan", fields: { lender: "Navy Federal", originalAmount: 35000, remainingBalance: 28750, apr: "4.29%", monthlyPayment: 650, term: "60 months", startDate: "2024-06-01" }, tags: ["loan", "vehicle"], notes: "Linked to Tesla Model 3.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(150), updated_at: now },
      { id: p8Id, user_id: uid, type: "investment", name: "Roth IRA", fields: { broker: "Fidelity", balance: 34500, contributions2026: 5000, maxContribution: 7000, holdings: "FXAIX, FZROX, FTIHX" }, tags: ["investment", "retirement"], notes: "Max out by end of year.", documents: [], linked_trackers: [], linked_expenses: [], linked_tasks: [], linked_events: [], created_at: d(100), updated_at: now },
    ]);

    // ---- Trackers + entries ----
    const t1Id = randomUUID(), t2Id = randomUUID(), t3Id = randomUUID(), t4Id = randomUUID(), t5Id = randomUUID();
    await this.supabase.from("trackers").insert([
      { id: t1Id, user_id: uid, name: "Weight", category: "health", unit: "lbs", icon: "Scale", fields: [{ name: "weight", type: "number", unit: "lbs", isPrimary: true }], linked_profiles: [], created_at: d(30) },
      { id: t2Id, user_id: uid, name: "Blood Pressure", category: "health", unit: "mmHg", icon: "Heart", fields: [{ name: "systolic", type: "number", unit: "mmHg", isPrimary: true }, { name: "diastolic", type: "number", unit: "mmHg" }, { name: "pulse", type: "number", unit: "bpm" }], linked_profiles: [], created_at: d(30) },
      { id: t3Id, user_id: uid, name: "Running", category: "fitness", unit: "miles", icon: "Footprints", fields: [{ name: "distance", type: "number", unit: "miles", isPrimary: true }, { name: "duration", type: "duration", unit: "mm:ss" }], linked_profiles: [], created_at: d(30) },
      { id: t4Id, user_id: uid, name: "Sleep", category: "health", unit: "hours", icon: "Moon", fields: [{ name: "hours", type: "number", unit: "hrs", isPrimary: true }], linked_profiles: [], created_at: d(20) },
      { id: t5Id, user_id: uid, name: "Food", category: "nutrition", unit: null, icon: "UtensilsCrossed", fields: [{ name: "item", type: "text", isPrimary: true }, { name: "calories", type: "number", unit: "kcal" }, { name: "protein", type: "number", unit: "g" }, { name: "carbs", type: "number", unit: "g" }, { name: "fat", type: "number", unit: "g" }], linked_profiles: [], created_at: d(15) },
    ]);

    const weightEntries = [186, 185.5, 185, 184.2, 184, 183.8, 183.5].map((w, i) => ({
      id: randomUUID(), user_id: uid, tracker_id: t1Id, entry_values: { weight: w }, computed: computeSecondaryData("Weight", "health", { weight: w }), notes: null, mood: null, tags: [], timestamp: d(7 - i),
    }));
    await this.supabase.from("tracker_entries").insert(weightEntries);

    const bpEntries = [{ s: 122, d: 78, p: 68, day: 5 }, { s: 118, d: 76, p: 72, day: 3 }, { s: 120, d: 80, p: 70, day: 1 }].map(bp => ({
      id: randomUUID(), user_id: uid, tracker_id: t2Id, entry_values: { systolic: bp.s, diastolic: bp.d, pulse: bp.p }, computed: computeSecondaryData("Blood Pressure", "health", { systolic: bp.s, diastolic: bp.d }), notes: null, mood: null, tags: [], timestamp: d(bp.day),
    }));
    await this.supabase.from("tracker_entries").insert(bpEntries);

    const runEntries = [{ dist: 3.2, dur: "28:00", day: 5 }, { dist: 2.5, dur: "22:30", day: 3 }, { dist: 4.0, dur: "35:20", day: 2 }, { dist: 3.0, dur: "26:15", day: 1 }].map(r => ({
      id: randomUUID(), user_id: uid, tracker_id: t3Id, entry_values: { distance: r.dist, duration: r.dur }, computed: computeSecondaryData("Running", "fitness", { distance: r.dist, duration: r.dur }), notes: null, mood: null, tags: [], timestamp: d(r.day),
    }));
    await this.supabase.from("tracker_entries").insert(runEntries);

    const sleepEntries = [{ h: 7.5, day: 4 }, { h: 6.0, day: 3 }, { h: 8.0, day: 2 }, { h: 7.0, day: 1 }].map(sl => ({
      id: randomUUID(), user_id: uid, tracker_id: t4Id, entry_values: { hours: sl.h }, computed: computeSecondaryData("Sleep", "health", { hours: sl.h }), notes: null, mood: null, tags: [], timestamp: d(sl.day),
    }));
    await this.supabase.from("tracker_entries").insert(sleepEntries);

    await this.supabase.from("tracker_entries").insert([
      { id: randomUUID(), user_id: uid, tracker_id: t5Id, entry_values: { item: "Chicken sandwich", calories: 450, protein: 35, carbs: 40, fat: 15 }, computed: computeSecondaryData("Food", "nutrition", { calories: 450, protein: 35, carbs: 40, fat: 15 }), notes: null, mood: null, tags: [], timestamp: d(2) },
      { id: randomUUID(), user_id: uid, tracker_id: t5Id, entry_values: { item: "Protein shake", calories: 280, protein: 40, carbs: 15, fat: 5 }, computed: computeSecondaryData("Food", "nutrition", { calories: 280, protein: 40, carbs: 15, fat: 5 }), notes: null, mood: null, tags: [], timestamp: d(1) },
    ]);

    // ---- Tasks ----
    const tk2Id = randomUUID(), tk3Id = randomUUID(), tk4Id = randomUUID();
    await this.supabase.from("tasks").insert([
      { id: randomUUID(), user_id: uid, title: "Renew driver's license", description: null, status: "todo", priority: "high", due_date: "2026-04-15", linked_profiles: [], tags: ["personal", "deadline"], created_at: d(10) },
      { id: tk2Id, user_id: uid, title: "Schedule vet appointment for Max", description: null, status: "todo", priority: "medium", due_date: null, linked_profiles: [p2Id], tags: ["pet", "max"], created_at: d(5) },
      { id: tk3Id, user_id: uid, title: "Pay car insurance", description: null, status: "done", priority: "high", due_date: null, linked_profiles: [p3Id], tags: ["vehicle", "finance"], created_at: d(15) },
      { id: tk4Id, user_id: uid, title: "Call Mom about Easter plans", description: null, status: "todo", priority: "medium", due_date: "2026-03-25", linked_profiles: [p1Id], tags: ["family"], created_at: d(3) },
      { id: randomUUID(), user_id: uid, title: "Research home gym equipment", description: null, status: "in_progress", priority: "low", due_date: null, linked_profiles: [], tags: ["fitness", "shopping"], created_at: d(7) },
    ]);
    await this.supabase.from("profiles").update({ linked_tasks: [tk2Id] }).eq("id", p2Id).eq("user_id", uid);
    await this.supabase.from("profiles").update({ linked_tasks: [tk3Id] }).eq("id", p3Id).eq("user_id", uid);
    await this.supabase.from("profiles").update({ linked_tasks: [tk4Id] }).eq("id", p1Id).eq("user_id", uid);

    // ---- Expenses ----
    const e2Id = randomUUID(), e3Id = randomUUID(), e4Id = randomUUID();
    await this.supabase.from("expenses").insert([
      { id: randomUUID(), user_id: uid, amount: 45.99, category: "food", description: "Grocery run", vendor: "Trader Joe's", is_recurring: false, linked_profiles: [], tags: ["groceries"], date: d(1), created_at: d(1) },
      { id: e2Id, user_id: uid, amount: 120, category: "pet", description: "Max - vet checkup", vendor: "Pacific Vet Clinic", is_recurring: false, linked_profiles: [p2Id], tags: ["vet", "max"], date: d(5), created_at: d(5) },
      { id: e3Id, user_id: uid, amount: 22.99, category: "entertainment", description: "Netflix - March", vendor: "Netflix", is_recurring: false, linked_profiles: [p4Id], tags: ["subscription"], date: d(2), created_at: d(2) },
      { id: e4Id, user_id: uid, amount: 65.00, category: "transport", description: "Tesla Supercharging", vendor: "Tesla", is_recurring: false, linked_profiles: [p3Id], tags: ["vehicle", "fuel"], date: d(4), created_at: d(4) },
      { id: randomUUID(), user_id: uid, amount: 35.50, category: "health", description: "Protein powder", vendor: "Amazon", is_recurring: false, linked_profiles: [], tags: ["fitness", "supplements"], date: d(3), created_at: d(3) },
    ]);
    await this.supabase.from("profiles").update({ linked_expenses: [e2Id] }).eq("id", p2Id).eq("user_id", uid);
    await this.supabase.from("profiles").update({ linked_expenses: [e4Id] }).eq("id", p3Id).eq("user_id", uid);
    await this.supabase.from("profiles").update({ linked_expenses: [e3Id] }).eq("id", p4Id).eq("user_id", uid);

    // ---- Events ----
    await this.supabase.from("events").insert([
      { id: randomUUID(), user_id: uid, title: "Annual Physical", date: "2026-07-15", time: "10:00 AM", end_time: null, end_date: null, all_day: false, description: "With Dr. Park", location: "Coastal Health Center", category: "health", color: null, recurrence: "yearly", recurrence_end: null, linked_profiles: [p5Id], linked_documents: [], tags: ["medical"], source: "manual", created_at: d(10) },
      { id: randomUUID(), user_id: uid, title: "Max's Vaccination", date: "2026-04-20", time: "2:00 PM", end_time: null, end_date: null, all_day: false, description: "Annual booster shots", location: "Pacific Vet Clinic", category: "family", color: null, recurrence: "yearly", recurrence_end: null, linked_profiles: [p2Id], linked_documents: [], tags: ["pet", "medical"], source: "manual", created_at: d(5) },
      { id: randomUUID(), user_id: uid, title: "Gym Session", date: dateStr(0), time: "6:00 AM", end_time: null, end_date: null, all_day: false, description: "Chest & triceps", location: null, category: "health", color: null, recurrence: "weekly", recurrence_end: null, linked_profiles: [], linked_documents: [], tags: ["fitness"], source: "manual", created_at: d(14) },
      { id: randomUUID(), user_id: uid, title: "Milo's Soccer Practice", date: dateStr(1), time: "4:00 PM", end_time: "5:30 PM", end_date: null, all_day: false, description: "At Carmel Valley fields", location: "Carmel Valley Park", category: "family", color: null, recurrence: "weekly", recurrence_end: null, linked_profiles: [], linked_documents: [], tags: ["kids", "sports"], source: "manual", created_at: d(3) },
      { id: randomUUID(), user_id: uid, title: "Sprint Planning", date: dateStr(2), time: "10:00 AM", end_time: "11:00 AM", end_date: null, all_day: false, description: "Q2 sprint kickoff", location: null, category: "work", color: null, recurrence: "biweekly", recurrence_end: null, linked_profiles: [], linked_documents: [], tags: ["work"], source: "manual", created_at: d(7) },
    ]);

    // ---- Documents ----
    const doc1Id = randomUUID(), doc2Id = randomUUID();
    await this.supabase.from("documents").insert([
      { id: doc1Id, user_id: uid, name: "Max - Vaccination Record", type: "medical", mime_type: "application/pdf", file_data: "", extracted_data: { petName: "Max", vaccines: ["Rabies", "DHPP", "Bordetella"], nextDue: "2026-04-20" }, linked_profiles: [p2Id], tags: ["pet", "medical"], created_at: d(60) },
      { id: doc2Id, user_id: uid, name: "Tesla Insurance Policy", type: "insurance", mime_type: "application/pdf", file_data: "", extracted_data: { provider: "State Farm", policyNumber: "SF-2024-789", coverage: "Full", premium: "$145/mo" }, linked_profiles: [p3Id], tags: ["insurance", "vehicle"], created_at: d(30) },
    ]);
    await this.supabase.from("profiles").update({ documents: [doc1Id] }).eq("id", p2Id).eq("user_id", uid);
    await this.supabase.from("profiles").update({ documents: [doc2Id] }).eq("id", p3Id).eq("user_id", uid);

    // ---- Habits + checkins ----
    const h1Id = randomUUID(), h2Id = randomUUID(), h3Id = randomUUID(), h4Id = randomUUID();
    await this.supabase.from("habits").insert([
      { id: h1Id, user_id: uid, name: "Drink 8 glasses of water", icon: "Droplets", color: "#4F98A3", frequency: "daily", target_days: null, current_streak: 5, longest_streak: 12, created_at: d(30) },
      { id: h2Id, user_id: uid, name: "Morning meditation", icon: "Brain", color: "#A84B2F", frequency: "daily", target_days: null, current_streak: 3, longest_streak: 7, created_at: d(20) },
      { id: h3Id, user_id: uid, name: "Read 30 minutes", icon: "BookOpen", color: "#6DAA45", frequency: "daily", target_days: null, current_streak: 0, longest_streak: 14, created_at: d(45) },
      { id: h4Id, user_id: uid, name: "No phone before bed", icon: "Smartphone", color: "#944454", frequency: "daily", target_days: null, current_streak: 2, longest_streak: 5, created_at: d(15) },
    ]);

    const habitCheckins: any[] = [];
    for (const ci of [{ day: 5, v: 8 }, { day: 4, v: 9 }, { day: 3, v: 8 }, { day: 2, v: 10 }, { day: 1, v: 8 }]) habitCheckins.push({ id: randomUUID(), user_id: uid, habit_id: h1Id, date: dateStr(ci.day), value: ci.v, notes: null, timestamp: d(ci.day) });
    for (const ci of [3, 2, 1]) habitCheckins.push({ id: randomUUID(), user_id: uid, habit_id: h2Id, date: dateStr(ci), value: null, notes: null, timestamp: d(ci) });
    for (const ci of [5, 4, 3]) habitCheckins.push({ id: randomUUID(), user_id: uid, habit_id: h3Id, date: dateStr(ci), value: null, notes: null, timestamp: d(ci) });
    for (const ci of [2, 1]) habitCheckins.push({ id: randomUUID(), user_id: uid, habit_id: h4Id, date: dateStr(ci), value: null, notes: null, timestamp: d(ci) });
    await this.supabase.from("habit_checkins").insert(habitCheckins);

    // ---- Obligations + payments ----
    const ob1Id = randomUUID(), ob2Id = randomUUID();
    await this.supabase.from("obligations").insert([
      { id: ob1Id, user_id: uid, name: "Rent", amount: 2200, frequency: "monthly", category: "housing", next_due_date: "2026-04-01", autopay: true, linked_profiles: [], notes: "Paid to Pacific Coast Properties", created_at: d(180) },
      { id: ob2Id, user_id: uid, name: "Tesla Loan Payment", amount: 650, frequency: "monthly", category: "loan", next_due_date: "2026-04-05", autopay: true, linked_profiles: [p3Id, p7Id], notes: "Navy Federal auto loan", created_at: d(150) },
      { id: randomUUID(), user_id: uid, name: "Car Insurance", amount: 145, frequency: "monthly", category: "insurance", next_due_date: "2026-04-01", autopay: false, linked_profiles: [p3Id], notes: "State Farm", created_at: d(90) },
      { id: randomUUID(), user_id: uid, name: "Gym Membership", amount: 49.99, frequency: "monthly", category: "health", next_due_date: "2026-04-01", autopay: true, linked_profiles: [], notes: "24 Hour Fitness", created_at: d(60) },
      { id: randomUUID(), user_id: uid, name: "Roth IRA Contribution", amount: 583, frequency: "monthly", category: "investment", next_due_date: "2026-04-01", autopay: true, linked_profiles: [p8Id], notes: "Monthly auto-invest to FXAIX", created_at: d(100) },
    ]);
    await this.supabase.from("obligation_payments").insert([
      { id: randomUUID(), user_id: uid, obligation_id: ob1Id, amount: 2200, date: d(30), method: "Bank transfer", confirmation_number: null },
      { id: randomUUID(), user_id: uid, obligation_id: ob1Id, amount: 2200, date: d(60), method: "Bank transfer", confirmation_number: null },
      { id: randomUUID(), user_id: uid, obligation_id: ob2Id, amount: 650, date: d(25), method: "Autopay", confirmation_number: null },
    ]);

    // ---- Artifacts ----
    await this.supabase.from("artifacts").insert([
      { id: randomUUID(), user_id: uid, type: "checklist", title: "Weekly Meal Prep", content: "", items: [{ id: randomUUID(), text: "Buy chicken breast (4 lbs)", checked: true, order: 0 }, { id: randomUUID(), text: "Prep rice and quinoa", checked: true, order: 1 }, { id: randomUUID(), text: "Wash and chop vegetables", checked: false, order: 2 }, { id: randomUUID(), text: "Cook and portion meals", checked: false, order: 3 }, { id: randomUUID(), text: "Prep protein shakes", checked: false, order: 4 }], tags: ["food", "health"], linked_profiles: [], pinned: true, created_at: d(3), updated_at: now },
      { id: randomUUID(), user_id: uid, type: "note", title: "Home Gym Equipment Research", content: "Looking at:\n- Power rack: Rogue RML-390F ($1,045)\n- Barbell: Rogue Ohio Bar ($295)\n- Bumper plates: 260lb set (~$500)\n- Adjustable dumbbells: Bowflex 552 ($349)\n- Flooring: Horse stall mats 4x6 ($45 each, need 6)\n\nTotal estimate: ~$2,500\n\nNeed to measure garage first. Min 8x8 ft for rack + platform.", items: [], tags: ["fitness", "shopping"], linked_profiles: [], pinned: false, created_at: d(7), updated_at: d(2) },
      { id: randomUUID(), user_id: uid, type: "checklist", title: "License Renewal Checklist", content: "", items: [{ id: randomUUID(), text: "Gather documents (ID, proof of address)", checked: true, order: 0 }, { id: randomUUID(), text: "Schedule DMV appointment online", checked: false, order: 1 }, { id: randomUUID(), text: "Take new photo", checked: false, order: 2 }, { id: randomUUID(), text: "Pay renewal fee ($38)", checked: false, order: 3 }], tags: ["personal", "deadline"], linked_profiles: [], pinned: true, created_at: d(5), updated_at: d(1) },
    ]);

    // ---- Journal Entries ----
    await this.supabase.from("journal_entries").insert([
      { id: randomUUID(), user_id: uid, date: dateStr(4), mood: "good", content: "Solid day. Got a good run in, meal prep is on track. Feeling focused on fitness goals.", tags: ["fitness", "productive"], energy: 4, gratitude: ["Good weather for running", "Healthy meal prep done"], highlights: ["Ran 3.2 miles", "Hit protein goal"], created_at: d(4) },
      { id: randomUUID(), user_id: uid, date: dateStr(3), mood: "neutral", content: "Average day. Work was busy, didn't have time for everything I planned. At least got meditation done.", tags: ["work", "busy"], energy: 3, gratitude: ["Stable income", "Good health"], highlights: ["Completed meditation streak day 2"], created_at: d(3) },
      { id: randomUUID(), user_id: uid, date: dateStr(2), mood: "amazing", content: "Great day! PR on my run, finished meal prep early. Mom called to check in. Feeling grateful and energized.", tags: ["fitness", "family", "productive"], energy: 5, gratitude: ["Mom's call", "Physical health", "Beautiful sunset"], highlights: ["New running PR", "Meal prep done", "8 hours of sleep"], created_at: d(2) },
      { id: randomUUID(), user_id: uid, date: dateStr(1), mood: "good", content: "Productive day at work. Made progress on the app project. Cooked a good dinner. Need to remember to drink more water.", tags: ["work", "coding"], energy: 4, gratitude: ["Coding progress", "Good dinner"], highlights: ["App dev session", "Protein shake post-workout"], created_at: d(1) },
    ]);

    // ---- Memory ----
    await this.supabase.from("memories").insert([
      { id: randomUUID(), user_id: uid, key: "favorite_food", value: "Chicken sandwiches and protein shakes", category: "preferences", created_at: d(10), updated_at: d(10) },
      { id: randomUUID(), user_id: uid, key: "workout_schedule", value: "Usually runs in the morning, gym 3-4x per week", category: "health", created_at: d(15), updated_at: d(5) },
      { id: randomUUID(), user_id: uid, key: "goal_weight", value: "Target weight: 180 lbs", category: "goals", created_at: d(20), updated_at: d(20) },
      { id: randomUUID(), user_id: uid, key: "height", value: "5'10\"", category: "facts", created_at: d(30), updated_at: d(30) },
      { id: randomUUID(), user_id: uid, key: "location", value: "San Diego, California", category: "facts", created_at: d(30), updated_at: d(30) },
    ]);
  }
}
