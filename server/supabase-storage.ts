...
export class SupabaseStorage implements IStorage {
  ...
  async getStats(filterProfileId?: string, filterProfileIds?: string[], opts?: { sharedFetches?: boolean }): Promise<DashboardStats> {
    ...
    // The same environment gate getDashboardEnhanced applies: QA rows never
    // reach the Settings tiles or /api/stats totals (shared/domain/data-environment).
    const expenses = forEnvironment(expenseSource.filter(e => passesProfileFilter(e.linkedProfiles, filterCtxExpense)));
    const trackers = forEnvironment(allTrackers.filter(t => matchesProfile(t.linkedProfiles)));
  ...
  async getDashboardEnhanced(filterProfileId?: string, filterProfileIds?: string[], opts?: { sharedFetches?: boolean; includeTestData?: boolean }): Promise<any> {
    ...
    // QA 2026-09-18 BUG-05 / F-13: month-to-date income is money that has
    // ARRIVED. The monthly equivalent of every stream counted a paycheck first
    // dated the 30th in full on the 18th, so INCOME · MTD, Cash Flow and the
