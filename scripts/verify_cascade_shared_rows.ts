...
async function main() {
  ...
  // 6. Cleanup: delete Alice, which should clean up the remaining shared rows.
  console.log(`\nCleaning up: deleting Alice (${alice.id.slice(0,8)}) ...`);
  await storage.deleteProfile(alice.id);

  // 7. Hard-delete any soft-deleted test rows so the database is left clean.
  for (const table of ["expenses", "incomes"]) {
    await sb.from(table).delete().eq("user_id", USER_ID).contains("tags", [TEST_TAG]);
  }
  // Final sanity: nothing tagged TEST_TAG should remain anywhere.
  for (const table of ["expenses", "tasks", "obligations", "events", "incomes"]) {
    const { data: leftover } = await sb.from(table).select("id").eq("user_id", USER_ID).contains("tags", [TEST_TAG]);
    if (leftover && leftover.length > 0) console.warn(`[cleanup] ${table} still has ${leftover.length} test rows`);
  }
