// tests/helpers/ai-sweep-scenarios.ts
//
// The canonical "every chat command actually runs" scenario list, shared by:
//   • tests/ai-executor-sweep.test.ts — drives executeTool directly
//   • tests/mcp-server.test.ts        — drives the same scenarios through a
//     real MCP client/server pair (server/mcp-server.ts)
// One list, two transports: if a scenario needs to change, both suites move
// together, and the exhaustiveness checks in each suite keep the list in
// lockstep with TOOL_DEFINITIONS.
import { expect } from "vitest";
import type { MemStorage } from "../../server/storage";

export const SWEEP_USER = "executor-sweep-user";
...
export const PROFILE_ID = (name: string) => `__PROFILE_ID:${name}__`;
...
export const SCENARIOS: Scenario[] = [
  ...
  {
    tool: "add_investment_holding", input: { account: "Roth IRA", symbol: "VOO", quantity: 10, price: 520 },
    verify: (res) => { expect(res.accountCreated).toBe(true); expect(res.holding.quantity).toBe(10); },
  },
  { tool: "add_investment_holding", input: { account: "Coinbase", symbol: "BTC", quantity: 0.05, costBasis: 3000 } },
  { tool: "update_investment_holding", input: { symbol: "VOO", changes: { price: 530 } } },
  { tool: "get_investment_summary", input: {} },
  { tool: "remove_investment_holding", input: { symbol: "BTC" } },
...
/**
 * Loan schedules only enter the app through the Finance → Loans import UI
 * (deliberately not a chat tool), so both suites seed one for the loan tools.
 */
export async function seedLoanSchedule(storage: MemStorage) {
...
/** Resolve __PROFILE_ID:Name__ placeholders using ids captured from creates. */
export function resolvePlaceholders(input: Record<string, any>, ids: Record<string, string>): Record<string, any> {
  const out = JSON.parse(JSON.stringify(input));
  for (const k of Object.keys(out)) {
    const m = typeof out[k] === "string" && out[k].match(/^__PROFILE_ID:(.+)__$/);
    if (m) out[k] = ids[m[1]] || `unresolved:${m[1]}`;
  }
  return out;
}

/** Capture profile / liability ids from create results for later scenarios. */
export function captureIds(tool: string, input: Record<string, any>, res: any, ids: Record<string, string>) {
  if (tool === "create_profile") {
    const id = res?.id || res?.profile?.id || res?.result?.id;
    if (id) ids[input.name] = id;
  }
  if (tool === "create_liability") {
    const id = res?.id || res?.liability?.id || res?.profile?.id || res?.result?.id;
    if (id) ids[`liability:${input.name}`] = id;
  }
}
