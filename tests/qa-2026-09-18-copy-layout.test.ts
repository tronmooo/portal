...
describe("F-58 copy agreement", () => {
  it("the activity line's noun agrees with its count", () => {
    ...
    // Both storages build the Recent Activity line through it.
    expect(read("server/storage.ts")).toContain("describeCount(nums[0][1], nums[0][0])");
    expect(read("server/supabase-storage.ts")).toContain("describeCount(nums[0][1], nums[0][0])");
  });
