...
describe("BUG-29 — bell messages use the human label, name the date once, and rank by real severity", () => {
  ...
  // Merged with F-51 (tests/qa-2026-09-18-ai-chat-search.test.ts): the shared
  // dateRuleNotice names the RECORD in the message, never the storage key.
  it("the document message reads '<record> expired N days ago (Jun 1, 2026)' — no field key, date once", async () => {
    ...
    expect(doc.message).toMatch(/^Homeowners Insurance Policy Declaration expired \d+ days ago \(Jun 1, 2026\)$/);
