...
//
// This module lives in shared/ (it was server-only until the Executive tab
// started reporting adherence gaps) and is PURE: no storage, no clock reads
// except the injectable `now` below. server/medication-doses.ts re-exports it
// so the AI engine's tool handlers are unaffected.
...
export function isMedicationTracker(t: Tracker): boolean {
 ...
 *  best signal wins. */
export function trackerFrequencyText(t: Tracker): string {
...
export interface DoseWindowOpts {
  /** Days back from today, default 7. */
  days?: number;
  /**
   * "Now" in epoch ms. Defaults to the wall clock. The Executive tab passes the
   * router's own `now` so a briefing rendered from a fixed clock — and the
   * tests that assert it — stay deterministic.
   */
  now?: number;
}
