...
/**
 * The ONE self profile the app treats as "me" (QA 2026-09-18 BUG-01).
 * Accounts can carry several self-typed rows (imports, seeders, an old "Me"
 * placeholder); `find()` on an unordered list returned a different one on
 * each fetch, so every re-seed could land on a different person. Same rule
 * as pickSelfProfile (the one implementation): the account's original —
 * oldest — Self, stable across fetches and tabs; `undefined` when there is
 * no honest answer (no Self, or several undated ones).
 */
export function pickPrimarySelf<T extends { id: string; type?: string; createdAt?: string | Date | null }>(profiles: ReadonlyArray<T>): T | undefined {
  return (pickSelfProfile(profiles) as T | null) ?? undefined;
}
...
type ProfileLite = { id: string; name?: string; type?: string; createdAt?: string | Date | null };

/** Epoch ms of a createdAt value (ISO string or Date); NaN when unusable. */
function createdAtMs(p: ProfileLite): number {
  if (!p.createdAt) return NaN;
  return new Date(p.createdAt as any).getTime();
}
...
export function pickSelfProfile(
  ...
  const dated = selfs.filter(p => Number.isFinite(createdAtMs(p)));
  if (dated.length !== selfs.length) return null;
  return [...dated].sort((a, b) => (createdAtMs(a) - createdAtMs(b)) || a.id.localeCompare(b.id))[0];
}
...
export async function reconcileProfileFilter(
  profiles: Array<{ id: string; name?: string; type?: string; createdAt?: string | Date | null }> | null | undefined,
  ...
  // re-map a live selection by name onto a different row — and is not worth
  // a verification round-trip either; a later, complete list will heal it.
 ...
 * name is exactly what the old reconcile then dropped. It is a label fix,
 * not a scope change, so it never warms a scope and must not look like one
 * to the rest of the app (QA 2026-09-18 BUG-01: MultiProfileFilter used
