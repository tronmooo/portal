...
export default function DashboardPage() {
  ...
  // QA 2026-09-18 F-01 / BUG-01: the "mirror the single selected profile back
  // into the store" effect that used to live here is gone. It re-ran on every
  // /api/profiles refetch (after any write) and wrote THIS page's React copy
  // of the scope into the store as `setFilterSelected([id], [name || ""])` —
  // so whenever the copy lagged the store (a switch made from the hub
  // switcher or another page a moment earlier), the stale value won and the
  // app silently flipped to the previous person, taking the record being
  // saved with it; and it stored an EMPTY name whenever the list happened to
  // lack the id, which is exactly what the old reconcile then dropped. The
  // store (lib/profileFilter.ts) is the only source of truth for the SCOPE;
  // this page only reads it. The one thing it may still refresh is the
  // selection's DISPLAY NAME after a rename: refreshFilterNames never touches
  // ids or mode and never writes an empty name.
  useEffect(() => {
