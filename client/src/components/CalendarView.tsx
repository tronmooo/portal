...
import { resolveOccurrenceTarget } from "@shared/occurrence-routing";
  ...
  const deleteMutation = useMutation<void, Error, void, { prevTimeline: [readonly unknown[], unknown][]; prevEntity: [readonly unknown[], unknown][]; entityKey: string }>({
    mutationFn: async () => {
      if (item.type === "event") {
        ...
        // A bill occurrence is ONE date generated by a liability's recurrence —
        // removing it here deletes just that date, not the liability. The write
        // goes to the liability (which owns the recurrence), so the profile's
        // Recurring Dates tab and every other surface drop it too. Previously
        // this SKIPPED the date, which left it on the calendar struck through
        // rather than removing it as the button promises.
        if (item.meta?.occurrenceId) {
          // occurrenceId is "<liabilityId>:<canonical date>". Use the canonical
          // date, not item.date — a rescheduled occurrence displays on its
          // moved date but is keyed by the original one.
          const occId = String(item.meta.occurrenceId);
          const sep = occId.indexOf(":");
          const liabilityId = sep > 0 ? occId.slice(0, sep) : (item.meta.liabilityId || item.sourceId);
          const canonicalDate = sep > 0 ? occId.slice(sep + 1, sep + 11) : item.date;
          await apiRequest("DELETE", `/api/liabilities/${liabilityId}/occurrences/${canonicalDate}`);
        } else {
          ...
          {item.type === "obligation" && (
            <Button
              ...
              onClick={(e) => {
                ...
                setLocation(resolveOccurrenceTarget({
                  sourceType: "liability",
                  sourceId: item.meta?.liabilityId || item.sourceId,
                  parentSection: "recurring",
            ...
            >
              <CreditCard className="h-3.5 w-3.5 mr-1" />Open {item.title}
            </Button>
          )}
...
export default function CalendarView({ externalFilterIds, externalFilterMode }: CalendarViewProps = {}) {
  ...
  const handleEditFromDetail = () => {
    ...
    // Obligations and tasks have no inline edit dialog here — the full form
    // lives on the object that OWNS the date. Route to that object, never to
    // the list it appears in: a bill occurrence is generated by a liability
    // profile, so editing it means opening that liability's Recurring Dates,
    // not dumping the user on the Bills page with no idea what they tapped.
    if (detailItem.type === "obligation") {
      const liabilityId = detailItem.meta?.liabilityId || detailItem.sourceId;
      setDetailItem(null);
      setLocation(resolveOccurrenceTarget({
        sourceType: "liability", sourceId: liabilityId, parentSection: "recurring",
        occurrenceDate: detailItem.date,
      }).path);
      return;
    }
    if (detailItem.type === "task") {
      setDetailItem(null);
      setLocation(resolveOccurrenceTarget({
        sourceType: "task", sourceId: detailItem.sourceId, occurrenceDate: detailItem.date,
      }).path);
      return;
    }
  };
